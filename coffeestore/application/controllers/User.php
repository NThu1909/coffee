...
class User extends MY_Controller {
	...
	public function register()
	{
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('email', 'Email Log in', 'required|valid_email|callback_check_email');
			...
			if ($this->form_validation->run()) {
				...
				if ($this->user_model->create($data)) {
					$this->session->set_flashdata('message_success', 'Register success');
				}else{
					$this->session->set_flashdata('message_fail', 'Register failure');
				}
	...
	function check_email()
	{
		...
		{
			$this->form_validation->set_message(__FUNCTION__,'Tên Log in đã tồn tại');
			return FALSE;
		}
		return TRUE;
	}
	public function login()
	{
		...
		$this->data['message_fail'] = $message_fail;
		if ($this->input->post()) {
			$this->form_validation->set_rules('email', 'Email Log in', 'required|valid_email');
			$this->form_validation->set_rules('password', 'Password', 'required');
