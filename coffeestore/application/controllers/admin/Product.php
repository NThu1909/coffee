...
class Product extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post())
		{
			...
			if ($flag > 0) {
				$this->session->set_flashdata('message_success', 'Delete'.$flag.'Product success');
			}else{
				$this->session->set_flashdata('message_fail', 'Delete Product failure');
			}
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->create($data)) {
					$this->session->set_flashdata('message_success', 'Add Product success');
				}else{
					$this->session->set_flashdata('message_fail', 'Add Product failure');
				}
				redirect(admin_url('product'));
			}
		}
	...
	public function edit()
	{
		...
		if (empty($product)) {
			$this->session->set_flashdata('message_fail', 'Product không tồn tại');
			redirect(admin_url('product'));
		}
		$this->data['product'] = $product; 
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->update($id,$data)) {
					$this->session->set_flashdata('message_success', 'Edit Product success');
				}else{
					$this->session->set_flashdata('message_fail', 'Edit Product failure');
				}
				redirect(admin_url('product'));
			}
		}
