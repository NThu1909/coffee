...
class slider extends MY_Controller {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->slider_model->create($data)) {
					$this->session->set_flashdata('message_success', 'Add slider success');
				}else{
					$this->session->set_flashdata('message_fail', 'Add slider failure');
				}
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->slider_model->update($id,$data)) {
					$this->session->set_flashdata('message_success', 'Edit slider success');
				}else{
					$this->session->set_flashdata('message_fail', 'Edit slider failure');
				}
				redirect(admin_url('slider'));
			}
		}
	...
	public function del()
	{
		...
		if ($this->slider_model->delete($id)) {
			...
			$this->session->set_flashdata('message_success', 'Delete Slider success');
		}else{
			$this->session->set_flashdata('message_fail', 'Delete Slider failure');
		}
		redirect(admin_url('slider'));
	}
}
