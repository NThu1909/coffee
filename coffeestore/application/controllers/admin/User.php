...
class User extends MY_Controller {
	...
	// 				$this->session->set_flashdata('message_success', 'Edit danh mục success');
	// 			}else{
	// 				$this->session->set_flashdata('message_fail', 'Edit danh mục failure');
	// 			}
	// 			redirect(user_url('user'));
	// 		}
	// 	}
	...
	public function accept()
	{
		...
		$this->session->set_flashdata('message_success', 'Confirm The order success');
	...
	public function del()
	{
		...
		if ($this->user_model->delete($id)) {
			$this->session->set_flashdata('message_success', 'Delete user success');
		}else{
			$this->session->set_flashdata('message_fail', 'Delete user failure');
		}
		redirect(user_url('user'));
	}
	public function deldetail()
	{
		...
		if (!$this->order_model->check_exists($where)) {
			$this->session->set_flashdata('message_fail', 'Directory does not exist');
			redirect(admin_url('transaction'));
		}
		if ($this->order_model->delete($id)) {
			$this->session->set_flashdata('message_success', 'Delete success');
		}else{
			$this->session->set_flashdata('message_fail', 'Delete failure');
		}
		redirect(admin_url('user'));
