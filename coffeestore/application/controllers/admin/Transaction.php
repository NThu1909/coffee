...
class Transaction extends MY_Controller {
	...
	public function del()
	{
		...
		if (empty($transaction)) {
			$this->session->set_flashdata('message_fail', 'The order không tồn tại');
			redirect(admin_url('transaction'));
		}
		$this->data['transaction'] = $transaction;
		if ($this->transaction_model->delete($id)) {
			$this->session->set_flashdata('message_success', 'Delete The order success');
		}else{
			$this->session->set_flashdata('message_fail', 'Delete The order failure');
		}
		redirect(admin_url('transaction'));
	}
